...
def run_migrations(registry, name, current_version, to_version, downgrade=False):
    ...
    sillyorm.migration_helpers.helper_init(
        registry, str(migration_dir), version_table=f"__silly_alembic_version_{name}"
    )

    _logger.info("running migrations of module %s to version %s", name, to_version)
    sillyorm.migration_helpers.helper_do_migrate(to_version, downgrade=downgrade)


def generate_migrations(registry, name=None, to_version=None):
    if not registry.get_schema_diffs():
    ...
    if not to_version:
        to_version = input(f"What is the new version of the module {name}: ")

    ask_msg = lambda: input(
        f"Migrations to be generated for module {name}! Please enter a migration message: "
    )

    if sillyorm.migration_helpers.helper_gen_migrations(ask_msg, to_version):
        raise Exception(
            f"migrations have been autogenerated for module {name}. Please review them! (THEY MAY"
            " CONTAIN TABLES FROM OTHER MODULES)"
        )
